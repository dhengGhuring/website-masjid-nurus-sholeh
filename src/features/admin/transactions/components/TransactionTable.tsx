...
export const TransactionTable = ({ transactions, isLoading, onEdit, onDelete }: TransactionTableProps) => {
    const { isOpen, onOpen, onOpenChange } = useDisclosure();
    const [selectedProof, setSelectedProof] = useState<string | null>(null);
    const { url: signedProofUrl, loading: signedProofLoading } = useTransactionProof(selectedProof);
    ...
    const renderCell = useCallback((transaction: any, columnKey: React.Key) => {
            ...
            case "actions":
                return (
                    <div className="flex gap-2 items-center">
                        <Button size="sm" color="primary" onPress={() => onEdit(transaction)} className="text-sm font-medium">Edit</Button>
                        <Button size="sm" color="danger" onPress={() => onDelete(transaction)} className="text-sm font-medium">Hapus</Button>
                    </div>
                );
            default:
                return cellValue;
                ...
                {!isLoading && transactions.length > 0 && (
                    ...
                    >
                        <TableHeader columns={columns}>
                            {(column) => (
                                <TableColumn key={column.uid} align={column.uid === "proof" || column.uid === "actions" ? "center" : "start"}>
                                    {column.name}
                                </TableColumn>
                            )}
                        </TableHeader>
