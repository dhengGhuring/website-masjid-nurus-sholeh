...
import { Button, Input, Select, SelectItem, DateRangePicker } from "@heroui/react";
...
export const TransactionsPage = () => {
    ...
    const startDate = dateRange?.start ? `${dateRange.start.year}-${String(dateRange.start.month).padStart(2, '0')}-${String(dateRange.start.day).padStart(2, '0')}` : null;
    const endDate = dateRange?.end ? `${dateRange.end.year}-${String(dateRange.end.month).padStart(2, '0')}-${String(dateRange.end.day).padStart(2, '0')}` : null;
            ...
            <main className="flex-1 p-8 overflow-y-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground">Kelola semua transaksi keuangan masjid</p>
                    </div>
                    <Button
                ...
                <TransactionTable
                    transactions={transactions?.data.filter((transaction) => transaction.deleted_at === null) || []}
                    isLoading={isLoading}
